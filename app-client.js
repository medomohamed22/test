...
function readProductsCache() {
  try {
    ...
    return Array.isArray(cached?.products) && Date.now()-Number(cached.savedAt||0)<5*60*1000 ? cached.products : null;
  } catch (_) { return null; }
}
...
async function loadAllProducts(){const cached=readProductsCache();if(cached){globalProducts=cached.slice();populateProductLocations();renderProducts();openProductFromUrlOnce()}else el('loading').style.display='block';try{const d=await api('public-products');globalProducts=d.products||[];writeProductsCache(globalProducts);populateProductLocations();renderProducts();openProductFromUrlOnce()}catch(e){if(!cached)safeSetHtml('products-list',`<div class="empty-state" style="grid-column:1/-1"><h3>خطأ</h3><p>${escapeHtml(e.message)}</p></div>`)}finally{el('loading').style.display='none'}}
