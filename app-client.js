...
function friendlyDbError(error) {
  if (error?.status === 404) {
    return currentLang === 'ar'
      ? 'مسار تسجيل الدخول غير موجود في آخر نشر على Vercel. ارفع مجلد api كامل ثم أعد النشر.'
      : 'The login API route is missing from the latest Vercel deployment.';
  }
  return error?.message || String(error || 'حدث خطأ غير متوقع');
}
