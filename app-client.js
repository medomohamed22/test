...
async function openMarketplacePanel(type){if(!user)return;const box=el('marketplace-panel');if(type==='favorites'){const fav=globalProducts.filter(p=>marketplaceState.favorites.has(Number(p.id)));box.innerHTML=`<div class="market-panel"><h4>${currentLang==='ar'?'المفضلة وعمليات البحث المحفوظة':'Favorites and saved searches'}</h4><div class="market-list">${fav.map(p=>`<div class="market-row"><span>${escapeHtml(p.name)}</span><button class="mini-btn mini-primary" onclick="openProductDetails(${p.id})">${currentLang==='ar'?'عرض':'View'}</button></div>`).join('')||`<small>${currentLang==='ar'?'لا توجد إعلانات مفضلة.':'No favorite ads.'}</small>`}${marketplaceState.savedSearches.map(x=>`<div class="market-row"><span><i class="fa-solid fa-bell"></i> ${escapeHtml(x.name)}</span><div class="mini-actions"><button class="mini-btn mini-primary" onclick="applySavedSearch(${x.id})">${currentLang==='ar'?'تطبيق':'Apply'}</button><button class="mini-btn mini-danger" onclick="deleteSavedSearch(${x.id})">${currentLang==='ar'?'حذف':'Delete'}</button></div></div>`).join('')}</div></div>`}
