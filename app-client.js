...
async function renewProduct(productId){if(!user)return showToast(t('toast_login_first'),'error');try{const pi=await ensurePiReady();const accessToken=await ensurePaymentAccessToken();const price=Number(piUsdPrice||0);if(!price)throw new Error(currentLang==='ar'?'تعذر تحديد سعر Pi الآن.':'Could not determine Pi price.');const amount=Number((1/price).toFixed(7));await Promise.resolve(pi.createPayment({amount,memo:`Renew Ad ${productId}`,metadata:{type:'renewal',productId:Number(productId)}},{onReadyForServerApproval:async paymentId=>{await api('approve',{method:'POST',headers:{...authHeaders(),Authorization:`Bearer ${accessToken}`},body:JSON.stringify({paymentId})})},onReadyForServerCompletion:async(paymentId,txid)=>{await api('complete',{method:'POST',headers:{...authHeaders(),Authorization:`Bearer ${accessToken}`},body:JSON.stringify({paymentId,txid})});showToast(t('renew_success'),'success');await Promise.all([loadAllProducts(true),loadMyAds()])},onCancel:()=>showToast(currentLang==='ar'?'تم إلغاء الدفع.':'Payment cancelled.','warning'),onError:e=>showToast(e?.message||(currentLang==='ar'?'فشل الدفع.':'Payment failed.'),'error')}))}catch(e){showToast(e.message,'error')}}
...
async function markAllNotificationsRead(){try{await mpApi('notification-read',{method:'POST',headers:authHeaders(),body:'{}'});await openMarketplacePanel('notifications')}catch(e){showToast(friendlyDbError(e),'error')}}
