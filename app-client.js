...
async function openMarketplacePanel(type){if(!user)return;const box=el('marketplace-panel');if(type==='favorites'){const fav=globalProducts.filter(p=>marketplaceState.favorites.has(Number(p.id)));box.innerHTML=`<div class="market-panel"><h4>المفضلة وعمليات البحث المحفوظة</h4><div class="market-list">${fav.map(p=>`<div class="market-row"><span>${escapeHtml(p.name)}</span><button class="mini-btn mini-primary" onclick="openProductDetails(${p.id})">عرض</button></div>`).join('')||'<small>لا توجد إعلانات مفضلة.</small>'}${marketplaceState.savedSearches.map(x=>`<div class="market-row"><span><i class="fa-solid fa-bell"></i> ${escapeHtml(x.name)}</span><div class="mini-actions"><button class="mini-btn mini-primary" onclick="applySavedSearch(${x.id})">تطبيق</button><button class="mini-btn mini-danger" onclick="deleteSavedSearch(${x.id})">حذف</button></div></div>`).join('')}</div></div>`}
 ...
 if(type==='verification'){const v=marketplaceState.verification||{};box.innerHTML=`<div class="market-panel"><h4>توثيق الحساب</h4><p>الحالة: <b>${escapeHtml(v.verification_status||'unverified')}</b></p><div class="mini-actions" style="margin-top:12px"><button class="btn btn-primary" onclick="requestVerification('phone')"><i class="fa-brands fa-telegram"></i> ${currentLang==='ar'?'إرسال رقم مرتبط بتيليجرام':'Submit Telegram-linked phone'}</button></div></div>`}
...
function openProductsMap(){const items=currentFilteredProducts().filter(p=>Number.isFinite(Number(p.latitude))&&Number.isFinite(Number(p.longitude)));document.querySelectorAll('.view-section').forEach(e=>e.classList.add('hidden'));el('view-map').classList.remove('hidden');const canvas=el('map-page-canvas'),grid=el('map-page-products');if(!items.length){canvas.innerHTML=`<div class="empty-state"><h3>${currentLang==='ar'?'لا توجد إعلانات بموقع تقريبي ضمن النتائج الحالية.':'No ads with approximate locations in the current results.'}</h3></div>`;grid.innerHTML='';return}const lats=items.map(x=>Number(x.latitude)),lngs=items.map(x=>Number(x.longitude)),minLat=Math.min(...lats)-.08,maxLat=Math.max(...lats)+.08,minLng=Math.min(...lngs)-.08,maxLng=Math.max(...lngs)+.08;canvas.innerHTML=`<iframe loading="lazy" referrerpolicy="no-referrer" src="https://www.openstreetmap.org/export/embed.html?bbox=${minLng},${minLat},${maxLng},${maxLat}&layer=mapnik"></iframe><div class="map-marker-layer">${items.map(x=>{const left=8+((Number(x.longitude)-minLng)/(maxLng-minLng))*84,top=8+(1-(Number(x.latitude)-minLat)/(maxLat-minLat))*84,img=x.images?.[0]||'https://placehold.co/100x100';return `<img class="map-photo-marker" style="left:${left}%;top:${top}%" src="${escapeAttr(img)}" alt="${escapeAttr(x.name)}" onclick="openProductDetails(${x.id})">`}).join('')}</div>`;grid.innerHTML=items.map(x=>`<article class="map-product-card" onclick="openProductDetails(${x.id})"><img src="${escapeAttr(x.images?.[0]||'https://placehold.co/300x300')}" loading="lazy" decoding="async"><div>${escapeHtml(x.name)}<small style="display:block;color:var(--text-muted)">${escapeHtml(x.country||'')} - ${escapeHtml(x.location||'')}</small></div></article>`).join('')}
function closeProductsMap(){nav('home')}

function openSingleProductMap(id){const p=globalProducts.find(x=>x.id==id);if(!p)return;document.querySelectorAll('.view-section').forEach(e=>e.classList.add('hidden'));el('view-map').classList.remove('hidden');const canvas=el('map-page-canvas'),grid=el('map-page-products'),lat=Number(p.latitude),lng=Number(p.longitude);canvas.innerHTML=`<iframe loading="lazy" referrerpolicy="no-referrer" src="https://www.openstreetmap.org/export/embed.html?bbox=${lng-.04},${lat-.04},${lng+.04},${lat+.04}&layer=mapnik&marker=${lat},${lng}"></iframe>`;grid.innerHTML=`<article class="map-product-card" onclick="openProductDetails(${p.id})"><img src="${escapeAttr(p.images?.[0]||'https://placehold.co/300x300')}"><div>${escapeHtml(p.name)}</div></article>`}
